...
function changeTask(taskName, videoSrc1, videoSrc2, videoSrc3) {
  ...
  if (currentTask === "tape") {
    // Iterate through the video labels and mark them as 2x
    const videoLabels = document.querySelectorAll('.video-label');
    videoLabels.forEach(label => {
      label.innerHTML = label.innerHTML.replace(/(\d+)x/, '2x');
    });
  }
  else {
...
document.addEventListener('DOMContentLoaded', (event) => {
  changeTask('lift', 'static/videos/lift_start.mp4', 'static/videos/lift_mid.mp4', 'static/videos/lift_last.mp4');
});
